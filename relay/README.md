...
This folder contains the public relay and push-service code used by Opendex pairing and trusted reconnect.

The point of keeping this code in the repo is transparency: anyone forking Opendex can inspect the transport boundary, verify the encrypted-session flow, and run a compatible relay of their own. What should stay private is the actual deployed endpoint and any production credentials.
...
- it does not decrypt Opendex application payloads after the secure session is established

OpenCode, git, and local file operations still run on the user's Mac.

## Security Model

Opendex uses the relay as a transport hop, not as a trusted application server.
...
- The relay does not receive plaintext Opendex application payloads after the secure session is active.
...
Push is disabled by default. Enable it only when you are ready to wire APNs and the bridge-side `OPENDEX_PUSH_SERVICE_URL`, for example with `OPENDEX_ENABLE_PUSH_SERVICE=true`.

## Deploy Notes

- Keep the real relay base URL in private config such as `OPENDEX_RELAY`, not in committed source.
- Keep APNs credentials in private env vars or protected files (`OPENDEX_APNS_*`).
- Leave `OPENDEX_TRUST_PROXY` unset for direct/self-hosted installs. Set it to `true` only when a trusted reverse proxy is forwarding requests to this relay.
- When `OPENDEX_TRUST_PROXY=true`, configure the proxy to send sanitized client IP headers (`X-Real-Ip` and/or appended `X-Forwarded-For`) instead of passing client-supplied values through unchanged.
- If you expose the relay under a shared-domain prefix such as `/opendex`, have the proxy strip that prefix before forwarding so the Node server still receives `/relay/...` and `/v1/push/...`.
