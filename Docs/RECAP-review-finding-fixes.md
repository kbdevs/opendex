...
1. The desktop refresher (`CodexDesktopRefresher`) now uses `hasPendingRefreshWork()` both before running and after finishing a refresh. This means a queued completion refresh survives the case where its timer fires during another slow refresh.
...
```mermaid
flowchart TD
    A["turn/completed arrives"] -->|marks pending completion| B["desktop refresher queueCompletionRefresh"]
