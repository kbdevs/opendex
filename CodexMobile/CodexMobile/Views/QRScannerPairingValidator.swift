...
private let qrScannerBridgeUpdateCommand = "bun add -g opendex@latest"
...
func validatePairingQRCode(_ code: String, now: Date = Date()) -> QRScannerPairingValidationResult {
    ...
    if looksLikeLegacyPairingPayload(data) {
        return .bridgeUpdateRequired(
            makeScannerBridgeUpdatePrompt(
                message: "This QR code looks like it came from an older Opendex bridge. Update the bridge on your Mac before scanning a new QR code."
            )
        )
    }

    return .scanError("Not a valid secure pairing code. Make sure you're scanning a QR from the latest Opendex bridge.")
}

private func looksLikeLegacyPairingPayload(_ data: Data) -> Bool {
